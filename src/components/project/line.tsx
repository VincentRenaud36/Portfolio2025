...
export default function Line() {
  ...
  const setPath = (progress: number) => {
    ...
    // const width = Math.min(window.innerWidth * 1, Number.MAX_SAFE_INTEGER);
    const width = window.innerWidth * 1;

    // Vérifier que x est une valeur valide
    if (!isFinite(x)) x = 0.5;

     // Vérifier que progress est une valeur valide
    ...
    path.current?.setAttributeNS(
      null,
      "d",
      `M0 250 Q${width * x} ${250 + safeProgress}, ${width} 250`
    );
  };
...
const id = params.id ? decodeURIComponent(params.id as string) : '';
  ...
  return (
    <div className="">
      <div className="flex">
        <Link href={'/'} className="text-2xl mr-20 inline-block">←</Link>
      </div>
        <div className="flex flex-col">
            <div className="text-4xl mb-10">{projet.nomProjet}</div>
            ...
            <div className="flex flex-col items-start mt-10">
                ...
                <div className="flex mt-12">
                    ...
                    <div className="flex w-full ml-8 flex-wrap gap-2.5">
                        <p className="border border-white rounded-full uppercase text-sm py-2.5 px-3">
                        {projet.tagProjet}
                        </p>
                    </div>
                </div>
